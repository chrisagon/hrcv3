...
function carriere_consultant_insert() {
	...
	$data['titre_emploi'] = $_REQUEST['titre_emploi'];
	...
	insert('carriere_consultant', backtick_keys_once($data), $error);
	if($error)
	...
	if(!empty($_REQUEST['SelectedID'])) carriere_consultant_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function carriere_consultant_copy_children($destination_id, $source_id) {
	global $Translation;
