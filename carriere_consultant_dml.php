...
function carriere_consultant_insert(){
	...
	sql("insert ignore into membership_userrecords set tableName='carriere_consultant', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function carriere_consultant_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process values
	if($selected_id){
		$templateCode=str_replace('<%%VALUE(id_carriere_conslt)%%>', html_attr($row['id_carriere_conslt']), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(titre_emploi)%%>', html_attr($row['titre_emploi']), $templateCode);
	...
	$templateCode .= loadView('carriere_consultant-ajax-cache', $cache_data);
