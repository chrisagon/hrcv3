...
function niveaux_ref_insert() {
	...
	insert('niveaux_ref', backtick_keys_once($data), $error);
	if($error)
	...
	if(!empty($_REQUEST['SelectedID'])) niveaux_ref_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function niveaux_ref_copy_children($destination_id, $source_id) {
	global $Translation;
