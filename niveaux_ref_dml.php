...
function niveaux_ref_insert(){
	...
	sql("insert ignore into membership_userrecords set tableName='niveaux_ref', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function niveaux_ref_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process values
	if($selected_id){
		$templateCode=str_replace('<%%VALUE(id_niveau)%%>', html_attr($row['id_niveau']), $templateCode);
	...
	$templateCode .= loadView('niveaux_ref-ajax-cache', $cache_data);
