<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on 'tags' table. 
...
function update_tags_list() {
	...
	$tags = array();
	$res = sql("select * from tags order by tags", $eo);
	while($row = db_fetch_assoc($res))
		$tags[] = $row['tags'];

	// save the tags to the options list file
	$list_file = dirname(__FILE__) . '/missions.tags.csv';
	@file_put_contents($list_file, implode(';;', $tags));
	$list_file = dirname(__FILE__) . '/competences_individuelles.tags.csv';
	@file_put_contents($list_file, implode(';;', $tags));
}