...
si OUI alors lire le CV pour alimenter les missions */
    /* chemin du CV */
    $chemin_cv = "../images/" . $data['cv_hrc'];
 echo "<br>documentCV = ".$chemin_cv."</br>";
 ...
 echo '<pre>'.print_r($tablo_danalyse, true);
...
// alim_missions_cv.php
    foreach ( $tablo_danalyse as $ligne ) {
        ...
        $environnement  = prendre_chaine_entre( $ligne,
            'ENVIRONNEMENT :', '/FINBLOK' );
        echo "missions_cv_insert = "  . $site_mission ."<br>". $periode."<br>". $objet_mission . "<br>" .
            $detail_mission ."<br>". $environnement."<br>" ;
        if (!empty($site_mission) && !empty($periode) && !empty($objet_mission) && !empty($detail_mssion) && !empty($environnement)) {
        missions_cv_insert( $data, $site_mission, $periode, $objet_mission,
            $detail_mission, $environnement );
        }
