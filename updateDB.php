<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('curriculum_vitae', "create table if not exists `curriculum_vitae` (   `id_cv` INT unsigned not null auto_increment , primary key (`id_cv`), `du_consultant` INT unsigned null , `Titre_du_cv` TEXT not null , `creer_par` VARCHAR(40) null ) CHARSET utf8", $silent);
		setupIndexes('curriculum_vitae', array('du_consultant'));
		setupTable('consultant', "create table if not exists `consultant` (   `id_consultant` INT unsigned not null auto_increment , primary key (`id_consultant`), `Matricule` VARCHAR(40) not null , `Prenom` VARCHAR(40) null , `Nom` VARCHAR(40) not null , `email` VARCHAR(80) null , `adresse_postale` TEXT null , `saisie_par` VARCHAR(40) null , `coache_par` INT unsigned null , `emploi_fonctionnel` INT unsigned null ) CHARSET utf8", $silent);
		setupIndexes('consultant', array('coache_par','emploi_fonctionnel'));
		setupTable('missions', "create table if not exists `missions` (   `id_mission` INT unsigned not null auto_increment , primary key (`id_mission`), `id_consultant` INT unsigned null , `rattache_a_cv` INT unsigned null , `date_debut` DATE not null , `date_fin` DATE null , `description_mission` TEXT not null , `description_detaille` TEXT null , `client` INT unsigned not null ) CHARSET utf8", $silent);
