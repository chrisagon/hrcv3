		...
		$pcConfig = array(
			...
			'missions' => array(   
				'id_consultant' => array(   
					...
					'forced-where' => '',
					'display-fields' => array(1 => 'Consultant', 3 => 'Date debut', 4 => 'Date fin', 5 => 'Description courte de la mission', 6 => 'Description detaille', 7 => 'Client'),
					'display-field-names' => array(1 => 'id_consultant', 3 => 'date_debut', 4 => 'date_fin', 5 => 'description_mission', 6 => 'description_detaille', 7 => 'client'),
					'sortable-fields' => array(0 => '`missions`.`id_mission`', 1 => '`missions`.`id_consultant`', 2 => '`missions`.`rattache_a_cv`', 3 => '`missions`.`date_debut`', 4 => '`missions`.`date_fin`', 5 => '`missions`.`description_mission`', 6 => '`missions`.`description_detaille`', 7 => '`missions`.`client`'),
					'records-per-page' => 10,
					'default-sort-by' => 3,
					...
					'query' => "SELECT `missions`.`id_mission` as 'id_mission', IF(    CHAR_LENGTH(`consultant1`.`Prenom`) || CHAR_LENGTH(`consultant1`.`Nom`), CONCAT_WS('',   `consultant1`.`Prenom`, ', ', `consultant1`.`Nom`), '') as 'id_consultant', IF(    CHAR_LENGTH(`consultant2`.`Prenom`) || CHAR_LENGTH(`consultant2`.`Nom`), CONCAT_WS('',   `consultant2`.`Prenom`, ', ', `consultant2`.`Nom`), '') as 'rattache_a_cv', if(`missions`.`date_debut`,date_format(`missions`.`date_debut`,'%d/%m/%Y'),'') as 'date_debut', if(`missions`.`date_fin`,date_format(`missions`.`date_fin`,'%d/%m/%Y'),'') as 'date_fin', `missions`.`description_mission` as 'description_mission', `missions`.`description_detaille` as 'description_detaille', IF(    CHAR_LENGTH(`client1`.`nom_du_client`), CONCAT_WS('',   `client1`.`nom_du_client`), '') as 'client' FROM `missions` LEFT JOIN `consultant` as consultant1 ON `consultant1`.`id_consultant`=`missions`.`id_consultant` LEFT JOIN `curriculum_vitae` as curriculum_vitae1 ON `curriculum_vitae1`.`id_cv`=`missions`.`rattache_a_cv` LEFT JOIN `consultant` as consultant2 ON `consultant2`.`id_consultant`=`curriculum_vitae1`.`du_consultant` LEFT JOIN `client` as client1 ON `client1`.`id_client`=`missions`.`client` "
				),
				'rattache_a_cv' => array(   
					'parent-table' => 'curriculum_vitae',
					'parent-primary-key' => 'id_cv',
					'child-primary-key' => 'id_mission',
					'child-primary-key-index' => 0,
					'tab-label' => 'Missions',
					'auto-close' => false,
			...
			'competences_individuelles' => array(   
				'rattache_a_mission' => array(   
					...
					'child-primary-key-index' => 0,
					'tab-label' => 'Competences individuelles',
					'auto-close' => false,
					...
					'sortable-fields' => array(0 => '`competences_individuelles`.`id_comp_indiv`', 1 => '`competences_individuelles`.`rattache_a_mission`', 2 => '`competences_individuelles`.`competence_mis_en_oeuvre`', 3 => '`competences_individuelles`.`niveau`', 4 => '`competences_individuelles`.`consultant_id`', 5 => '`competences_individuelles`.`Documents_capitalises`', 6 => '`competences_individuelles`.`commentaires`'),
					'records-per-page' => 10,
			...
			'domaine' => array(   
				'rattache_a_filiere' => array(   
					...
					'sortable-fields' => array(0 => '`domaine`.`id_domaine`', 1 => '`domaine`.`titre_domaine`', 2 => '`domaine`.`description`', 3 => '`domaine`.`rattache_a_filiere`'),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
