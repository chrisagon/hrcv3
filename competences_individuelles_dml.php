...
function competences_individuelles_insert(){
	...
	sql("insert ignore into membership_userrecords set tableName='competences_individuelles', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function competences_individuelles_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process values
	if($selected_id){
		$templateCode=str_replace('<%%VALUE(id_comp_indiv)%%>', html_attr($row['id_comp_indiv']), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(id_comp_indiv)%%>', urlencode($urow['id_comp_indiv']), $templateCode);
		$templateCode=str_replace('<%%VALUE(rattache_a_mission)%%>', html_attr($row['rattache_a_mission']), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(rattache_a_mission)%%>', urlencode($urow['rattache_a_mission']), $templateCode);
		$templateCode=str_replace('<%%VALUE(competence_mis_en_oeuvre)%%>', html_attr($row['competence_mis_en_oeuvre']), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(Documents_capitalises)%%>', html_attr($row['Documents_capitalises']), $templateCode);
	...
	$templateCode .= loadView('competences_individuelles-ajax-cache', $cache_data);
