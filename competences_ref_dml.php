...
function competences_ref_insert(){
	...
	sql("insert ignore into membership_userrecords set tableName='competences_ref', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function competences_ref_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		if($dvprint){
			$templateCode = str_replace('<%%VALUE(titre_competence)%%>', nl2br(html_attr($row['titre_competence'])), $templateCode);
		}else{
		...
		$templateCode=str_replace('<%%VALUE(domaine_principal)%%>', html_attr($row['domaine_principal']), $templateCode);
	...
	$templateCode .= loadView('competences_ref-ajax-cache', $cache_data);
