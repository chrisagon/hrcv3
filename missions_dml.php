...
function missions_insert(){
	...
	sql("insert ignore into membership_userrecords set tableName='missions', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function missions_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process values
	if($selected_id){
		$templateCode=str_replace('<%%VALUE(id_mission)%%>', html_attr($row['id_mission']), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(rattache_a_cv)%%>', html_attr($row['rattache_a_cv']), $templateCode);
		...
		if($dvprint){
			$templateCode = str_replace('<%%VALUE(description_mission)%%>', nl2br(html_attr($row['description_mission'])), $templateCode);
		}else{
	...
	$templateCode .= loadView('missions-ajax-cache', $cache_data);
