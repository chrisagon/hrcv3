...
function domaine_insert() {
	...
	$data = array();
	$data['titre_domaine'] = $_REQUEST['titre_domaine'];
	...
	insert('domaine', backtick_keys_once($data), $error);
	if($error)
	...
	if(!empty($_REQUEST['SelectedID'])) domaine_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function domaine_copy_children($destination_id, $source_id) {
	global $Translation;
